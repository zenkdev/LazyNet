...
{
    partial class RepairsFilter : FilterControlBase
    {
        bool _allowDataSourceChanged = true;
        public RepairsFilter()
            : base(new FilterTreeSettings<AppSettings>(AppSettings.Default, x => x.RepairsStaticFilters, x => x.RepairsCustomFilters))
        {
            InitializeComponent();
        }
        ...
        {
            var frm = new RepairFilterForm(item);
            if (frm.ShowDialog(ParentFormMain) == DialogResult.OK)
            {
                if (frm.Save)
                {
        ...
        public string GetFilterName(object filtersCollection, FilterItemBase filter, bool focused)
        {
            if (filter != null)
            {
                ...
                {
                    count = session.GetObjects(session.GetClassInfo<Repair>(), filter.FilterCriteria, null, 0, false, false).Count;
                }
                if (count > 0)
