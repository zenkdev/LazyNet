...
{
    partial class SoftwareFilter : FilterControlBase
    {
        bool _allowDataSourceChanged = true;
        public SoftwareFilter()
            : base(new FilterTreeSettings<AppSettings>(AppSettings.Default, x => x.SoftwareStaticFilters, x => x.SoftwareCustomFilters))
        {
            InitializeComponent();
        }
        ...
        public override void New()
        {
            ...
            var frm = new RepairFilterForm(newFilterItem);
        ...
        {
            var frm = new SoftwareFilterForm(item);
            if (frm.ShowDialog(ParentFormMain) == DialogResult.OK)
            {
                if (frm.Save)
                {
        ...
        void RaiseDataSourceChanged(FilterItemBase filterItem)
        {
            ...
            handler?.Invoke(treeList, new DataSourceChangedEventArgs(filterItem.Name, filterItem.FilterCriteria));
        }
        ...
        public string GetFilterName(object filtersCollection, FilterItemBase filter, bool focused)
        {
            if (filter != null)
            {
                ...
                {
                    count = session.GetObjects(session.GetClassInfo<SoftwareObject>(), filter.FilterCriteria, null, 0, false, false).Count;
                }
                if (count > 0)
